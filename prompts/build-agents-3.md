# 20 MORE Agent Build Prompts (Batch 3: Prompts 41–60)

> Continuation of [build-agents.md](build-agents.md) and [build-agents-2.md](build-agents-2.md). Each prompt is self-contained. Paste one per chat session.

---

## PROMPT 41 — AI Chat / Ask Page (RAG-Powered Q&A)
...
- Headings use font-serif; body uses font-sans; code uses font-mono
...
API ROUTES:
- `/api/ask` — Accepts POST { question: string }, returns AI-generated answer with cited sources
- `/api/rag` — RAG (Retrieval-Augmented Generation) endpoint
- `/api/ai` — General AI endpoint
- `/api/summarize` — Text summarization

EXISTING HOOK: `src/hooks/useAIStream.ts` — a hook for streaming AI responses. Check its interface.

TASK: Build an AI-powered "Ask" page — a ChatGPT-like Q&A interface for crypto questions.

### PAGE: `src/app/[locale]/ask/page.tsx` ("use client")

### COMPONENTS TO CREATE:

1. **`src/components/AIChatInterface.tsx`** ("use client"):
   - Chat-style UI with messages (user messages right-aligned, AI responses left-aligned)
   - Input bar at the bottom: textarea (auto-growing) + Send button
   - Streaming response: use `useAIStream` hook if it supports streaming, otherwise implement SSE/fetch streaming manually
   - AI messages rendered with Markdown support (headers, lists, bold, code blocks)
   - Citation badges at bottom of AI responses linking to source articles
   - Loading indicator: animated dots while AI is thinking
   - Keyboard: Enter to send, Shift+Enter for new line
   - Max 50 messages in conversation
   - "New Chat" button to clear conversation

2. **Suggested Questions** — Show when chat is empty:
...
3. **AI Disclaimer** — Small footer text: "AI responses are generated from recent crypto news. Not financial advice."

4. **Share Chat** — Copy conversation as text button

### FILES TO CREATE:
- `src/app/[locale]/ask/page.tsx`
- `src/components/AIChatInterface.tsx`
- `src/components/MarkdownRenderer.tsx` (render AI markdown responses safely, no external deps - use regex-based parsing)

After changes, run `bun run build` to verify. Commit: "feat: add AI-powered ask/chat page with streaming responses"
```

---

## PROMPT 42 — Podcast & AI Anchor Page
...
DESIGN SYSTEM: CSS vars (--color-surface, --color-accent, --color-border, --color-surface-secondary), .container-main, font-serif headings, cn(), UI: Button/Badge/Card/Skeleton, Icons: lucide-react

API ROUTES:
- `/api/podcast` — Podcast episodes data (title, description, audio URL, duration, date)
- `/api/ai-anchor` — AI-generated audio news broadcasts
- `/api/summarize` — Summarization endpoint

TASK: Create Podcast & AI Audio News page.

### PAGE: `src/app/[locale]/podcast/page.tsx`

SECTIONS:
1. **Featured Episode** — Large card with play button for the latest episode:
   - Title, description, date, duration
   - Inline audio player (use native HTML5 `<audio>` with custom styled controls)

2. **Audio Player Component** — Create `src/components/AudioPlayer.tsx` ("use client"):
   - Custom-styled audio player (not browser default)
   - Play/pause button, progress bar (clickable to seek), time display (current / total), volume slider
   - Playback speed: 0.5x, 1x, 1.5x, 2x buttons
   - Styled to match design system (accent color for progress, surface backgrounds)
   - Mini-player mode: when scrolling down, show a sticky mini player at the bottom with title + play/pause

3. **Episode List** — Scrollable list of past episodes:
   - Each: title, date, duration, play button, description (truncated)
   - Click to expand and show full description
   - Pagination

4. **AI News Briefing** — Special section for AI-generated audio briefings:
   - "Today's Crypto Briefing" generated by AI
   - Play button with waveform visualization (CSS animated bars)
...
After changes, run `bun run build` to verify. Commit: "feat: add podcast page with custom audio player and AI briefings"
```
