...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: article.title,
    description:
      article.description || `Read the full article from ${article.source}`,
    alternates: {
...
export default async function ArticlePage({ params }: Props) {
  ...
  const readingTime = article.meta?.word_count
    ? Math.max(1, Math.ceil(article.meta.word_count / 200))
    : null;

  // Use SEO-friendly slug for URL, fallback to id if no slug
  const articleSlug =
    article.slug ||
    generateArticleSlug(article.title, article.pub_date || article.first_seen);
  ...
  const breadcrumbs = [
    ...
    {
      name: article.source,
      url: `https://cryptocurrency.cv/source/${article.source_key}`,
    },
    {
      name:
              ...
              <article className="bg-white dark:bg-black rounded-2xl border border-gray-200 dark:border-neutral-800 shadow-sm overflow-hidden">
                <div className="p-6 md:p-8">
                  ...
                  <div className="flex flex-wrap items-center gap-3 mb-4">
                    ...
                    <span
                      className={`text-sm px-3 py-1 rounded-full ${sentiment.color}`}
                    >
                  ...
                  <div className="space-y-3">
                    ...
                    {article.market_context.sol_price && (
                      <div className="flex justify-between items-center p-3 bg-purple-50 dark:bg-purple-900/30 rounded-lg">
                        ...
                        <span className="font-bold text-gray-900 dark:text-white">
                          {formatPrice(article.market_context.sol_price)}
                        </span>
                      </div>
                    )}
                    {article.market_context.total_market_cap && (
                      <div className="flex justify-between items-center p-3 bg-gray-50 dark:bg-black rounded-lg">
                        ...
                        <span className="font-bold text-gray-900 dark:text-white">
                          $
                          {(
                            article.market_context.total_market_cap / 1e12
                          ).toFixed(2)}
                          T
                        </span>
                      </div>
