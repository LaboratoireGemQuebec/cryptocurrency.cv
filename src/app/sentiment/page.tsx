      ...
      <div className="max-w-6xl mx-auto">
        ...
        <main className="px-4 py-8">
          <div className="mb-8 text-center">
            <h1 className="text-4xl font-bold mb-2">🎯 Market Sentiment</h1>
          ...
          {data ? (
              ...
              <div className="bg-white rounded-2xl border border-gray-200 p-8">
                <div className="grid md:grid-cols-2 gap-8 items-center">
                  <div className="text-center">
                    <h2 className="text-lg text-gray-500 mb-4">Market Sentiment Score</h2>
                    <SentimentGauge score={data.market.score} />
                    <div className="mt-12">
                      <span className={`inline-flex items-center gap-2 px-6 py-3 rounded-full text-lg font-bold ${sentimentColors[data.market.overall]?.bg || 'bg-gray-100'} ${sentimentColors[data.market.overall]?.text || 'text-gray-600'}`}>
                  ...
                  <div>
                    ...
                    <p className="text-gray-700 mb-4">{data.market.summary}</p>
                    
                    <h3 className="font-bold text-lg mb-3">Key Drivers</h3>
              ...
              <div>
                ...
                <div className="space-y-3">
                  {data.articles?.map((article, i) => (
                    <div 
                      key={i} 
                      className={`bg-white rounded-xl border p-5 ${sentimentColors[article.sentiment]?.border || 'border-gray-200'}`}
                    >
                      <div className="flex items-start justify-between gap-4 mb-3">
                        ...
                        <div className="flex flex-col items-end gap-2">
                          <span className={`px-3 py-1 rounded-full text-sm font-medium ${sentimentColors[article.sentiment]?.bg || 'bg-gray-100'} ${sentimentColors[article.sentiment]?.text || 'text-gray-600'}`}>
                            {sentimentEmojis[article.sentiment] || '➡️'} {article.sentiment.replace('_', ' ')}
                          </span>
                          <span className={`text-xs px-2 py-0.5 rounded ${article.impactLevel === 'high' ? 'bg-red-100 text-red-700' : article.impactLevel === 'medium' ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-600'}`}>
                            {article.impactLevel} impact
                          </span>
                        </div>
                      </div>
                      <p className="text-sm text-gray-600 mb-3">{article.reasoning}</p>
            ...
            <div className="text-center py-16 bg-white rounded-xl">
              ...
              <h3 className="text-xl font-semibold text-gray-700 mb-2">Sentiment Analysis Unavailable</h3>
