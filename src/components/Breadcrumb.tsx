  ...
  return (
    <>
      ...
      <nav
        ...
        className="container-main py-3 border-b border-[var(--color-border)]"
      >
        <ol className="flex items-center gap-1.5 text-sm text-[var(--color-text-tertiary)] overflow-x-auto">
          {allItems.map((item, index) => {
            ...
            return (
              <li key={index} className="flex items-center gap-1.5 shrink-0">
                ...
                {isLast ? (
                  <span className="font-medium text-[var(--color-text-primary)] truncate max-w-[200px]" aria-current="page">
                    {item.label}
                  </span>
                ) : item.href ? (
                  <Link
