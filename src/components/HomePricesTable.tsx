...
function Sparkline({ prices, width = 64, height = 24 }: { prices: number[]; width?: number; height?: number }) {
  if (!prices || prices.length < 2) return <span className="text-gray-300 dark:text-gray-400 text-xs">—</span>;
...
function ColumnCustomizer({
  ...
  return (
    ...
    >
      <div className="bg-white dark:bg-black rounded-2xl shadow-2xl border border-gray-200 dark:border-neutral-800 w-full max-w-2xl max-h-[90vh] overflow-y-auto"
        role="dialog"
        aria-modal="true"
                      ...
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all ${
                          active
                            ...
                            : 'bg-white dark:bg-black border-gray-200 dark:border-neutral-800 text-gray-700 dark:text-gray-300 hover:border-blue-400 dark:hover:border-blue-500'
        ...
        {/* Footer */}
        <div className="flex items-center justify-between p-5 border-t border-gray-100 dark:border-neutral-800 bg-gray-50/50 dark:bg-black/30 rounded-b-2xl">
...
export default function HomePricesTable({ limit = 20 }: { limit?: number }) {
  ...
  const sortIcon = (field: SortField | undefined) => {
    ...
    if (sortField !== field) return (
      <svg className="w-3 h-3 text-gray-300 dark:text-gray-400 inline ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="bg-white dark:bg-black rounded-2xl border border-gray-100 dark:border-neutral-800 overflow-hidden">
        <div className="animate-pulse">
          {Array.from({ length: 10 }).map((_, i) => (
            <div key={i} className="flex items-center gap-4 px-4 py-3.5 border-b border-gray-50 dark:border-neutral-800/50">
              <div className="w-6 h-4 bg-gray-100 dark:bg-black rounded" />
              <div className="w-7 h-7 bg-gray-100 dark:bg-black rounded-full" />
              <div className="flex-1 h-4 bg-gray-100 dark:bg-black rounded" />
              <div className="w-24 h-4 bg-gray-100 dark:bg-black rounded" />
              <div className="w-16 h-4 bg-gray-100 dark:bg-black rounded" />
              <div className="w-20 h-4 bg-gray-100 dark:bg-black rounded" />
            </div>
          ))}
        </div>
      </div>
  ...
  if (error) {
    return (
      <div className="bg-white dark:bg-black rounded-2xl border border-gray-100 dark:border-neutral-800 p-8 text-center">
  ...
  return (
    <>
      ...
      <div className="bg-white dark:bg-black rounded-2xl border border-gray-100 dark:border-neutral-800 overflow-hidden">
        {/* Toolbar */}
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-gray-100 dark:border-neutral-800 bg-gray-50/60 dark:bg-black/60">
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-xs font-medium text-gray-600 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors border border-gray-200 dark:border-neutral-800 rounded-lg px-3 py-1.5 hover:border-blue-300 dark:hover:border-blue-600"
          >
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-max text-sm">
            <thead>
              <tr className="border-b border-gray-100 dark:border-neutral-800 bg-gray-50/40 dark:bg-black/40">
                {/* Fixed: rank */}
                <th className="sticky left-0 z-10 bg-gray-50/90 dark:bg-black/90 backdrop-blur-sm text-left text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider px-4 py-3 w-10">#</th>
                {/* Fixed: name */}
                <th className="sticky left-10 z-10 bg-gray-50/90 dark:bg-black/90 backdrop-blur-sm text-left text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider px-4 py-3 min-w-[160px]">Name</th>
                {/* Dynamic columns */}
                {activeCols.map(col => (
                  <th
                    key={col.id}
            ...
            <tbody className="divide-y divide-gray-50 dark:divide-neutral-800/50">
              {sortedCoins.map((coin, idx) => (
                <tr key={coin.id} className="group hover:bg-gray-50 dark:hover:bg-neutral-900/30 transition-colors">
                  {/* Rank */}
                  <td className="sticky left-0 z-10 bg-white dark:bg-black group-hover:bg-gray-50 dark:group-hover:bg-slate-700/30 transition-colors px-4 py-3 text-xs font-bold text-gray-300 dark:text-gray-400 tabular-nums w-10">
                    {coin.market_cap_rank ?? idx + 1}
                  </td>
                  {/* Name */}
                  <td className="sticky left-10 z-10 bg-white dark:bg-black group-hover:bg-gray-50 dark:group-hover:bg-slate-700/30 transition-colors px-4 py-3 min-w-[160px]">
                    <Link href={`/coin/${coin.id}`} className="flex items-center gap-2.5 min-w-0 hover:opacity-80 transition-opacity">
                      {coin.image ? (
                        ...
                        <span className="w-6 h-6 flex items-center justify-center text-xs font-bold text-gray-500 bg-gray-100 dark:bg-black rounded-full flex-shrink-0">
        ...
        {/* Footer */}
        <div className="px-4 py-3 border-t border-gray-100 dark:border-neutral-800 bg-gray-50/50 dark:bg-black/50 flex items-center justify-between">
