...
function generateMockMetrics(symbol: string): SocialMetrics {
  const baseVolume = Math.random() * 100000;
  ...
  return {
    ...
    name: symbol,
    timestamp: new Date(),
    socialVolume: Math.round(baseVolume),
    socialVolumeChange24h: (Math.random() - 0.5) * 100,
    socialDominance: Math.random() * 10,
    sentiment,
    sentimentChange24h: (Math.random() - 0.5) * 0.4,
    bullishPercent: 50 + sentiment * 25,
    bearishPercent: 50 - sentiment * 25,
    twitterFollowers: Math.round(Math.random() * 1000000),
    twitterFollowersChange24h: (Math.random() - 0.5) * 5,
    twitterMentions: Math.round(Math.random() * 10000),
    redditSubscribers: Math.round(Math.random() * 500000),
    redditActiveUsers: Math.round(Math.random() * 5000),
    telegramMembers: Math.round(Math.random() * 100000),
    influencerMentions: Math.floor(Math.random() * 20),
    topInfluencers: generateMockInfluencers(3),
    socialVolumeToPrice: (Math.random() - 0.5) * 2,
    sentimentToPrice: (Math.random() - 0.5) * 2,
    galaxyScore: Math.round(Math.random() * 100),
    altRank: Math.floor(Math.random() * 500) + 1,
  };
}

function generateMockInfluencers(count: number): InfluencerMention[] {
  const influencers = [
    { username: 'crypto_whale', platform: 'twitter' as const, followers: 850000 },
    { username: 'defi_chad', platform: 'twitter' as const, followers: 420000 },
    { username: 'altcoin_daily', platform: 'youtube' as const, followers: 1200000 },
    { username: 'coin_bureau', platform: 'youtube' as const, followers: 2100000 },
    { username: 'satoshi_stan', platform: 'twitter' as const, followers: 180000 },
  ];

  return influencers.slice(0, count).map(inf => ({
    ...inf,
...
function generateMockTrends(limit: number): SocialTrend[] {
  const coins = [
  ...
  return coins.slice(0, limit).map((coin, index) => ({
    ...coin,
    trendScore: 100 - index * 3 + Math.random() * 5,
    volumeSpike: Math.random() > 0.7,
    sentimentShift: Math.random() > 0.8,
    reason: [
      'High social volume',
      'Trending on Twitter',
      'Major announcement',
