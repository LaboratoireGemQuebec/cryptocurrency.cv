...
const ARG_LIMITS: Record<string, Record<string, { min?: number; max?: number }>> = {
...
function parseArgValue(tokens: string[], pos: { i: number }): unknown {
  ...
  if (!t) return null;
  if (t.startsWith('"')) { pos.i++; return t.slice(1, -1); }
  if (t === '[') {
    pos.i++;
    const arr: unknown[] = [];
...
function parseSelectionSet(tokens: string[], pos: { i: number }): string[] {
  ...
  while (pos.i < tokens.length && !(tokens[pos.i] === '}' && depth === 0)) {
    ...
    if (tokens[pos.i] === '}') { depth--; pos.i++; continue; }
    if ('(),:'.includes(tokens[pos.i])) { pos.i++; continue; }
...
export function parseQuery(query: string): ParsedField[] {
  ...
  if (tokens[pos.i] === 'query' || tokens[pos.i] === 'mutation' || tokens[pos.i] === 'subscription') {
    pos.i++;
...
export function calculateComplexity(parsedFields: ParsedField[]): number {
  ...
  for (const field of parsedFields) {
    ...
    const isListField = ['news', 'breaking', 'search', 'prices', 'trending', 'whales'].includes(field.name);
...
export function validateArgs(parsedFields: ParsedField[]): ValidationError[] {
  ...
  for (const field of parsedFields) {
    ...
    for (const [argName, value] of Object.entries(field.args)) {
      if (typeof value === 'string' && /<script|javascript:|on\w+\s*=/i.test(value)) {
        errors.push({
...
export function validateQuery(query: string): QueryValidationResult {
  ...
  if (depth > MAX_QUERY_DEPTH) {
    errors.push({ message: `Query depth ${depth} exceeds maximum allowed depth of ${MAX_QUERY_DEPTH}` });
  }

  // 2. Parse
  const parsedFields = parseQuery(query);
  if (parsedFields.length === 0) {
    errors.push({ message: 'Could not parse query. Ensure it follows GraphQL syntax: { fieldName(args) { subFields } }' });
    return { ok: false, errors, depth };
  }

  // 3. Alias / field count check
  if (parsedFields.length > MAX_ALIASES) {
    errors.push({ message: `Too many root fields (${parsedFields.length}). Maximum is ${MAX_ALIASES}` });
  }

  // 4. Complexity check
  const complexity = calculateComplexity(parsedFields);
  if (complexity > MAX_QUERY_COMPLEXITY) {
    errors.push({ message: `Query complexity ${complexity} exceeds maximum allowed cost of ${MAX_QUERY_COMPLEXITY}` });
  }
